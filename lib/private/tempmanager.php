...
class TempManager implements ITempManager {
	...
	public function getTempBaseDir() {
		...
		$directories = [];
		if ($temp = $this->config->getSystemValue('tempdirectory', null)) $directories[] = $temp;
		if ($temp = ini_get('upload_tmp_dir')) $directories[] = $temp;
		if ($temp = getenv('TMP')) $directories[] = $temp;
		if ($temp = getenv('TEMP')) $directories[] = $temp;
		if ($temp = getenv('TMPDIR')) $directories[] = $temp;
		$temp = tempnam(__FILE__, '');
		if (file_exists($temp)) {
			unlink($temp);
			$directories[] = dirname($temp);
		}
		if ($temp = sys_get_temp_dir()) $directories[] = $temp;
